...
from population_analysis import get_sample_info_df, compute_population_df, count_alleles, plot_allele_count, compute_cumulative_allele_classification, plot_cumulative_allele_classification
...
def main():
    ...
    parser_pop.add_argument("count_alleles", help="Count alleles in dataframe")
    parser_pop.add_argument("output_allele_counts", help="output directory for allele counts")
    parser_pop.add_argument("output_allele_counts_plot", help="output file for allele counts plot")
    ...
    if args.command == "compute-population-df":
        ...
        pop_df = compute_population_df(args.vcf_dir, sample_info, args.cores, output=args.output_pop_df)
        pop_df.to_csv(args.output_pop_df, index=False)
        ...
        pop_df = pd.read_csv(args.population_df_file)
        CN_allele_counts = count_alleles(pop_df, column="CN", sample_size=args.sample_size, output=os.path.join(args.output_allele_counts, "CN_allele_counts.csv"))
        len_allele_counts = count_alleles(pop_df, column="len_", sample_size=args.sample_size, output=os.path.join(args.output_allele_counts, "len_allele_counts.csv"))
        motif_allele_counts = count_alleles(pop_df, column="motifs", sample_size=args.sample_size, output=os.path.join(args.output_allele_counts, "motif_allele_counts.csv"))
        plot_allele_count(CN_allele_counts, len_allele_counts, motif_allele_counts, output=args.output_allele_counts_plot)
        ...
        pop_df = pd.read_csv(args.population_df_file)
        sample_info=get_sample_info_df(args.sample_info_file)
        cumm_df = compute_cumulative_allele_classification(pop_df, sample_info, output=args.output_cum_allele)
